...
import { updatePaper } from '@/app/services/actions-papers';
import { useFormState } from 'react-dom';
import { PapersTable } from '@/app/lib/models';

export default function Form({ paper }: { paper: PapersTable }) {
  const updatePaperWithId = updatePaper.bind(null, paper.id);
  const initialState = { message: null, errors: {} };
  const [state, dispatch] = useFormState(updatePaperWithId, initialState);
  ...
  return (
    <form action={dispatch}>
          ...
          <div className="relative mt-2 rounded-md">
            <div className="relative">
              <input
                ...
                type="textarea"
                placeholder="请输入总分"
                defaultValue={paper.total_score}
                className="peer block w-full rounded-md border border-gray-200 py-2 pl-4 text-sm outline-2 placeholder:text-gray-500"
                required
      ...
      <div className="mt-6 flex justify-end gap-4">
        ...
        <Button type="submit">更新试卷</Button>
      </div>
    </form>
  );
}
